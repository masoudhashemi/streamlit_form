...
def md_report(df, checkboxes_textboxes, report, template, image_template, folder_name):
    ...
    for i, row in df.iterrows():
        ...
        if pd.isna(row["Image"]):
            ...
            image_template_text = image_template.format(image=f"{folder_name}/images/{row['Image']}", caption=row["Caption"])
    ...
    with open(f"{folder_name}/report.md", "w") as f:
        f.write(report)
    ...
    if page == "Form":
        with st.spinner("Loading..."):
            ...
            if st.button("Save"):
                ...
                for i in range(len(checkboxes_textboxes)):
                    ...
                    if not pd.isna(images[i]):
                        ...
                        save_folder = f"./{folder_name}/images"
            ...
            if st.button("Generate Report"):
                ...
                md_report(df, checkboxes_textboxes, report, template, image_template, folder_name)
                st.success("Report successfully generated.")


# Show the CSV content page
if page == "CSV Content":
    csv_file = st.selectbox("Select a CSV file", [f"{folder_name}/data.csv"])
    if csv_file:
